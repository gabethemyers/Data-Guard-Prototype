...
from sqlalchemy import Boolean, ForeignKey, Numeric, String, UUID, text
...
class StagedDish(Base):
    ...
    meal_timing: Mapped[list[str] | None] = mapped_column(
        ARRAY(String),
        nullable=True,
        server_default=text("'{}'"),
    )
    ...
    taste_active_overall_confidence: Mapped[float | None] = mapped_column(
        Numeric(3, 2),
        nullable=True,
        default=0,
    )
